"""Venue Matching Agent - Searches and scores venues."""

import json
from typing import Any

from src.agents.base import BaseAgent, AgentResponse
from src.config.prompts import VENUE_MATCHING_PROMPT
from src.config.settings import settings
from src.tools.schemas import VENUE_TOOLS
from src.tools.registry import execute_tool
from src.observability.tracer import tracer
from src.models.trace import TraceEventType


class VenueMatchingAgent(BaseAgent):
    """Agent specialized in finding and matching venues."""

    def __init__(self, client=None):
        """Initialize the venue matching agent."""
        super().__init__(
            name="venue_matching",
            system_prompt=VENUE_MATCHING_PROMPT,
            tools=VENUE_TOOLS,
            client=client
        )
    ...
    def process(
        ...
        """Process a user message and search for venues.

        Args:
            user_message: The user's query about venues
            context: Optional context (preferences, etc.)
            conversation_history: Optional conversation history

        Returns:
            AgentResponse with venue recommendations
        """
        tracer.record_event(
        ...
        self.logger.info("Processing venue matching request", query_length=len(user_message))
